<script lang="ts">
  ...
  let matchedCount = $derived(result ? result.matched.filter((m) => m.diffs.length === 0).length : 0);
...
<div class="sidebar-content">
  ...
  {#if !result && !error && !loading}
    <div class="info-section">
      <span class="compare-hint">Select two hosts and click Compare to see differences between environments.</span>
    </div>
  {/if}
</div>
