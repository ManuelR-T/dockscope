...
`);
  ...
  it('parses depends_on array form', async () => {
    const path = writeTmp('deps-array.yml', `
services:
  web:
    image: nginx
    depends_on:
...
`);
  ...
  it('parses depends_on extended form', async () => {
    const path = writeTmp('deps-extended.yml', `
services:
  web:
    image: nginx
    depends_on:
...
`);
  ...
  it('parses networks', async () => {
    const path = writeTmp('networks.yml', `
services:
  web:
    image: nginx
    networks:
...
`);
  ...
  it('parses environment as array', async () => {
    const path = writeTmp('env-array.yml', `
services:
  app:
    image: node
    environment:
...
`);
  ...
  it('parses environment as object', async () => {
    const path = writeTmp('env-obj.yml', `
services:
  app:
    image: node
    environment:
...
`);
  ...
  it('parses labels as array', async () => {
    const path = writeTmp('labels-array.yml', `
services:
  app:
    image: node
    labels:
...
`);
  ...
  it('parses healthcheck', async () => {
    const path = writeTmp('healthcheck.yml', `
services:
  db:
    image: postgres
    healthcheck:
...
`);
  ...
  it('parses resource limits', async () => {
    const path = writeTmp('resources.yml', `
services:
  app:
    image: node
    deploy:
...
`);
  ...
  it('defaults image to name:latest when not specified', async () => {
    const path = writeTmp('no-image.yml', `
services:
  myapp:
    build: .
`);
