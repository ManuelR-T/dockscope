...
export function listHosts(): { name: string; url: string; connected: boolean; containers: number; version: string }[] {
...
export async function refreshHostStatus(): Promise<void> {
  await Promise.all(
    [...hosts.values()].map(async (h) => {
      try {
        const info = await Promise.race([h.client.info(), rejectAfter<{ ServerVersion?: string }>(3000)]);
        const list = await Promise.race([h.client.listContainers({ all: true }), rejectAfter<Dockerode.ContainerInfo[]>(3000)]);
